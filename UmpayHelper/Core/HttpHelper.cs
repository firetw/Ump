...
namespace WebLoginer
{
    public class HttpHelper
    {
        ...
        private string RedirectCookies = "";//重定向过程中产生的Cookie
        #endregion
        ...
        Redirect:
            ...
            try
            {
                ...
                using (response = (HttpWebResponse)request.GetResponse())
                {
                    ...
                    if (response.Headers["Set-Cookie"] != null) result.Cookie = Utilities.ControlCookies(response.Headers["Set-Cookie"]);
                    if (response.Headers["Location"] != null)//重定向链接
                    {
                        result.Location = response.Headers["Location"];
                        #region 如果允许重定向,则自动执行重定向链接直到结束
                        if (!item.Allowautoredirect)
                        {
                            item.Method = "GET";
                            item.URL = result.Location;
                            item.Cookie += result.Cookie;
                            RedirectCookies += result.Cookie;
                            IsRedirect = true;
                            goto Redirect;
                        }
                        #endregion
                    ...
                    else
                    {
                        ...
                        if (ResponseByte != null & ResponseByte.Length > 0)
                        {
                            ...
                            {
                                if (encoding == null) encoding = GetEncoding(ResponseByte);//未指定编码时自动获取网页编码
            ...
            #region 如果允许重定向,返回重定向过程中产生的Cookie
            if (IsRedirect) result.Cookie += RedirectCookies;
            #endregion
        ...
        private Encoding GetEncode(string charset)
        {
            ...
            switch (charset)
            {
                ...
                default: return Encoding.GetEncoding(response.CharacterSet.ToUpper());
            }
        }
        #endregion
        ...
        private void SetRequest(HttpItem item)
        {
            ...
            if (item.Method.ToLower().Contains("post")) request.ContentType = item.ContentType;//ContentType返回类型
        ...
        private void SetCookie(HttpItem item)
        {
            ...
            {
                if (item.Cookie.EndsWith("; ")) item.Cookie = item.Cookie.Substring(0, item.Cookie.Length - 2);
